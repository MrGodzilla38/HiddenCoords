...
A minimal **client-side Fabric mod** that displays your coordinates, dimension, and biome in a separate desktop window — completely outside Minecraft's own game window.
...
- ⚔️ **SMP / anarchy server players** who want to check their coordinates privately without opening the in-game debug screen (F3) on a shared or recorded screen.
- 🖱️ Anyone who just wants a small, always-available coordinate readout without cluttering their HUD.
...
| Feature | Description |
|---|---|
| 🪟 Separate window | Coordinates render in a **real, separate desktop window** (Java Swing) — not an in-game overlay, so screen/window capture software won't pick it up |
| ⌨️ Toggle key | Show/hide the window with **F6** by default (fully rebindable in Minecraft's Controls menu) |
| 📍 Live info | Displays **X / Y / Z**, current **dimension** (Overworld / Nether / End), and current **biome** |
| 💾 Remembers state | Window position and size are saved between sessions |
| 🖥️ Always on Top | Optional always-on-top behavior |
| 🛡️ Fails safely | If no display/graphics environment is available (e.g. headless setups) or the window can't be created, the mod disables the overlay and logs a warning instead of crashing the game |
| ✅ Client-side only | No server-side installation required, safe to use on any server |
...
See [MIT](https://github.com/MrGodzilla38/HiddenCoords/tree/main?tab=MIT-1-ov-file) for details.
